...
The platform team's Jenkins-based deploy pipeline is shared across 3 squads (~40 developers). Deploys are disproportionately slow for small changes: a recent config-file-only change took 47 minutes to staging, with 35 minutes spent in the integration test stage running the full suite. A separate concern about Friday-specific pipeline breakage was raised initially but is explicitly parked — this one-pager covers only the slow-deploy problem.
...
- **Current workarounds (all brittle):**
  ...
  - Behavioural avoidance: some devs stop pushing small changes on Fridays and wait until Monday — invisible productivity tax.
  - A 2025 hackathon prototype of selective testing worked technically but was never productionised — no owner emerged.
...
- Org-wide pipeline standardisation beyond the 3 squads currently sharing this pipeline.
...
Software / Tool evaluation — specifically test-impact-analysis or selective-testing tooling that integrates with Jenkins. Justification: the constraint set (Jenkins, audit-acceptable selectivity, no dedicated owner) narrows the candidate space, and a prior in-house attempt failed on ownership rather than technology, so an evaluation that explicitly weighs sustainment is the right next move.
